...
# 디버깅을 위한 nginx deploy 를 배포하기
resource "kubernetes_deployment_v1" "nginx" {
  # eks 가 준비 완료 되었을때 실행되도록 depends_on 을 추가한다
  depends_on = [ module.eks ]
  # yaml 팡리 대신에 terraform 표현식으로 deploy 할수 있다.
  metadata {
    name   = "nginx-hello"
    labels = { app = "nginx" }
  }
...
# terraform 으로 service 배포
resource "kubernetes_service_v1" "nginx_svc" {
  depends_on = [ module.eks, kubernetes_deployment_v1.nginx ]
  ...
  spec {
    ...
    # type = "LoadBalancer" 라고 하면 자동으로 LoadBalancer 가 프로비저닝 되고 서비스에 붙는다
    type = "LoadBalancer"
  }
}

# 만들어진 LoadBalancer 의 접근 주소 출력하기
output "lb_url" {
