...
# 클러스터 접속정보 (local k8s 를 바라보도록 context 가 변경되어 있어야 한다)
provider "kubernetes" {
...
resource "argocd_application" "microservice_app"{
    metadata {
      ...
      # argocd 가 거주하고 있는 namespace에 제출하고,
      namespace = "argocd"
    }
    spec {
        project = "default"
        source {
            # helm repository 주소 또는 github 주소도 가능
            repo_url = "https://hacksungboo.github.io/helm-microservice/"
            # 설치할 chart 의 이름
            chart = "msa-platform"
            # 버전 
            target_revision = "0.1.3"
        }
        destination {
            # 정해진 이름 
            server = "https://kubernetes.default.svc"
...
resource "argocd_application" "prometheus_stack"{
    ...
    spec {
        ...
        destination {
            # 정해진 이름 
            server = "https://kubernetes.default.svc"
            namespace = "monitoring" # 배포할 namespace 지정 
        }
        # 동기화 정책
        sync_policy {
            automated {
            ...
            # argocd 는 용량 제한이 있기때문에 k8s에 직접 던져서 실행이 되도록
