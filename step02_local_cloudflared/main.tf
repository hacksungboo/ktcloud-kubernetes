...
resource "cloudflare_tunnel_config" "vmware_config" {
  ...
  config {
    ...
    ingress_rule {
      # ccmall.shop 로 요청이 들어오면 터널과 연결된 아래의 위치로 전달한다
...
resource "null_resource" "install_cloudflared_pod" {
    ...
    # aws 에 프로비저닝 하는것이 아닌 local 에서 직접 ansible 플레이북을 실행
    ...
    # terraform destroy 했을때 deploy 와 secret 도 같이 삭제 되도록 한다.connection {
    provisioner "local-exec"{
      when = destroy
      command = <<-EOF
